...
interface ParsedRow extends BulkMenuItemInput {
  _valid: boolean;
  _errors: string[];
}
...
export function MenuImportDialog({ open, onOpenChange }: MenuImportDialogProps) {
  ...
  const handleDownloadTemplate = () => {
    const template = getMenuCSVTemplate();
  ...
  const handleImport = () => {
    ...
    startTransition(async () => {
      const itemsToImport: BulkMenuItemInput[] = validItems.map(({ _valid, _errors, ...item }) => item);
      const result = await bulkCreateMenuItems(itemsToImport);

      if (result.success) {
        toast.success(`Successfully imported ${result.created} menu items`);
        onOpenChange(false);
        resetForm();
        router.refresh();
      } else {
        toast.error(result.error || "Failed to import menu items");
      }
    });
  };
    ...
    <Dialog open={open} onOpenChange={(o) => { if (!o) resetForm(); onOpenChange(o); }}>
      <DialogContent className="max-w-4xl max-h-[90vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Upload a CSV file to bulk import menu items. Download the template for the correct format.
          </DialogDescription>
        </DialogHeader>
