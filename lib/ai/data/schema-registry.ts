...
export const SCHEMA_REGISTRY: SchemaEntity[] = [
  ...
  },
  {
    entity: 'inventory',
    description:
      'Branch on-hand stock (retail / kitchen), reorder points, and categories—separate from central warehouse hub inventory',
    fields: [
  ...
  },
  {
    ...
    fields: [
      ...
      { name: 'currentStock', type: 'decimal', description: 'Quantity at hub' },
      { name: 'transferQuantity', type: 'decimal', description: 'Quantity sent to a branch' },
    ...
    timeRanges: ['today', 'this_week', 'this_month', 'last_30_days'],
    requiredRole: [
      Role.SUPER_ADMIN,
      Role.ADMIN,
      Role.EXECUTIVE,
  ...
  },
  {
    ...
    description:
      'Unified order management: sources (call center, online, WhatsApp, walk-in, POS), types (dine-in, takeout, delivery), lifecycle status, payments, and delivery',
    fields: [
      ...
      { name: 'customer', type: 'reference', description: 'Linked customer', referenceTo: 'customers' },
    ],
    allowedAggregations: ['sum', 'count'],
    allowedGroupings: ['status', 'source', 'branch', 'paymentStatus'],
    timeRanges: ['today', 'this_week', 'this_month', 'last_30_days'],
    requiredRole: [
      Role.SUPER_ADMIN,
      Role.ADMIN,
  ...
  {
    entity: 'posOrders',
    description: 'In-venue POS terminal orders (quick-service tickets) with channels and line items, distinct from dashboard Sale aggregates',
    fields: [
      { name: 'orderNumber', type: 'string', description: 'POS ticket number' },
      { name: 'sourceChannel', type: 'enum', description: 'POS channel', enumValues: ['DINE_IN', 'TAKEOUT', 'DELIVERY', 'APP'] },
      { name: 'total', type: 'currency', description: 'Ticket total' },
      { name: 'status', type: 'enum', description: 'Ticket status', enumValues: ['NEW', 'IN_PROGRESS', 'READY', 'COMPLETED', 'CANCELLED'] },
      { name: 'branch', type: 'reference', description: 'Branch', referenceTo: 'branch' },
    ],
    allowedAggregations: ['sum', 'count'],
    allowedGroupings: ['sourceChannel', 'status', 'branch'],
    timeRanges: ['today', 'this_week', 'this_month'],
    requiredRole: [
      Role.SUPER_ADMIN,
      Role.ADMIN,
      Role.EXECUTIVE,
  ...
  {
    entity: 'customers',
    description: 'CRM customers (phone, profile) linked to orders and engagement',
    fields: [
      { name: 'name', type: 'string', description: 'Customer name' },
      { name: 'phone', type: 'string', description: 'Primary phone' },
      { name: 'orders', type: 'count', description: 'Related orders' },
    ],
    allowedAggregations: ['count'],
    allowedGroupings: ['city'],
    requiredRole: [
      Role.SUPER_ADMIN,
      Role.ADMIN,
      Role.EXECUTIVE,
  ...
  },
  {
    ...
    description:
      'Exported Excelite POS reports (Excel with frozen headers): executive performance, weekly digest, kitchen efficiency, menu performance, waste variance, customer insights (PII masked for non-admins), POS terminal, plus sales, inventory, warehouse, staff, orders—not a live query surface',
    fields: [
      { name: 'reportName', type: 'string', description: 'Report title' },
      { name: 'period', type: 'string', description: 'User-selected date range' },
      { name: 'format', type: 'enum', description: 'Export format', enumValues: ['CSV', 'EXCEL'] },
    ],
    allowedAggregations: [],
    allowedGroupings: [],
    requiredRole: [
